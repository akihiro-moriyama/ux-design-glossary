...
[価値]:#価値 "ある事物が人間に与える満足の度合い"

note:｢価値｣という言葉を｢ユーザーに価値を感じさせる要素｣の意味で使うことがある
...
[ジョブ理論]の用語で、ある特定のシチュエーションで[ユーザー]が成し遂げたい進捗

[ジョブ]:#ジョブ "ジョブ理論の用語で、ある特定のシチュエーションでユーザーが成し遂げたい進捗"

note:｢ジョブ｣は大目標を達成するための具体的で小規模な中間目標という意味合いがある
...
note:｢単体の製品サービスはあくまでもジョブをこなすためのものである｣｢単体の製品サービスだけでユーザーの大目標は達成できない｣｢企業はユーザーの大目標にこそ着目すべき｣というのがジョブ理論の趣旨。
...
[ナッジ]を悪用し、企業に都合の良い行動([ユーザー]自身にとっては不利な行動)を[ユーザー]に取らせるよう誘導する行為

[スラッジ]:#スラッジ "ナッジを悪用し、企業に都合の良い行動(ユーザー自身にとっては不利な行動)をユーザーに取らせるよう誘導する行為"
...
心理学的知見を悪用し、企業に都合の良い行動([ユーザー]自身にとっては不利な行動)を[ユーザー]に取らせるよう誘導するデザインの総称

→(類義語)[スラッジ]

[ダークパターン]:#ダークパターン "心理学的知見を悪用し、企業に都合の良い行動(ユーザー自身にとっては不利な行動)をユーザーに取らせるよう誘導するデザインの総称"
...
<1>UXデザインの用語｢[ユーザータスク]｣の省略語 <2>UXデザインの用語｢[評価タスク]｣の省略語 <3>設計業務において、設計者に割り振られる作業

[タスク]:#タスク "<1>UXデザインの用語｢ユーザータスク｣の省略語 <2>UXデザインの用語｢評価タスク｣の省略語 <3>設計業務において、設計者に割り振られる作業"

### タッチポイント

サービスデザインの用語で、企業と[ユーザー]との接点の総称(宣伝、店舗、店員、包装、製品自体、等)

[タッチポイント]:#タッチポイント "企業とユーザーとの接点(宣伝、店舗、店員、包装、製品自体、等)"
...
｢近接・整列・反復・対比｣の4つ。印刷物やウェブページ等の[デザイン]において、美観と情報伝達に優れた視覚的表現を実現するための基本的な約束事

[デザインの4つの基本原則]:#デザインの4つの基本原則 "｢近接・整列・反復・対比｣の4つ。印刷物やウェブページ等のデザインにおいて、美観と情報伝達に優れた視覚的表現を実現するための基本的な約束事"
...
note:特徴分析の作業工程はほぼKJ法と同一である。｢ユーザーの典型的な特徴｣というテーマでKJ法を行い、KJ法の工程の最後である論文化工程の代わりに選択回答式アンケート風に集計する点だけが異なる
...
[ユーザビリティ評価]における、評価対象とする[ユーザータスク]

[評価タスク]:#評価タスク "ユーザビリティ評価における、評価対象とするユーザータスク"

note:ユーザーテストにおいては、評価タスクがテスト協力者の行う作業課題となる

note:ヒューリスティック評価や認知的ウォークスルーにおいては、まず検査者が評価対象物を用いて評価タスクを行なった上で、評価対象物の問題点を探す
...
note:品質の定義における｢要求｣とはユーザー要求のことである。企業側の製品サービスへの要求(例えば予算や日程)は管理されるべきビジネス上の重要指標だが、品質の定義からは外れる

note:企業に都合の良い方向にユーザーの心理や行動を誘導する目的でUXをデザインする場合がある。どの程度実際に誘導されるかは管理されるべきビジネス上の重要指標だが、品質の定義からは外れる
...
[ユーザー調査]で得た典型的な[ユーザー]の[基本属性]・過去/現在の[体験]・[利用文脈]を図表でわかりやすく表現する作業

[ユーザーモデリング]:#ユーザーモデリング "ユーザー調査で得た典型的なユーザーの基本属性・過去/現在の体験・利用文脈を図表でわかりやすく表現する作業"
...
[アジャイル開発]の用語で、[製品サービス]との関わり方の違いによって分類された[ユーザー]の属性。たとえば業務用システムの場合は｢現場担当者｣｢現場監督者｣｢IT管理者｣｢経営者｣など。

[ユーザーロール]:#ユーザーロール "[アジャイル開発]の用語で、[製品サービス]との関わり方の違いによって分類された[ユーザー]の属性。たとえば業務用システムの場合は｢現場担当者｣｢現場監督者｣｢IT管理者｣｢経営者｣など。"

note:ユーザーロールは｢ペルソナ｣に類似の概念であるが、さも実在の人物であるかのような表現はユーザーロールでは必須ではない
