...
2つの試作物の優劣比較を目的とした[ユーザーテスト]。特に、2つの広告案のどちらがより高い広告成果を得られるか（クリック数増加やページビュー数増加など）を検証することを目的としたユーザーテスト

[A/Bテスト]:#A/Bテスト "2つの試作物の優劣比較を目的としたユーザーテスト。特に、2つの広告案のどちらがより高い広告成果を得られるかを検証することを目的としたユーザーテスト"

[ABテスト]:#A/Bテスト "（→A/Bテスト）2つの試作物の優劣比較を目的としたユーザーテスト。特に、2つの広告案のどちらがより高い広告成果を得られるかを検証することを目的としたユーザーテスト"

note:一般的にユーザーテストは試作段階で行われることが多いが、A/Bテストは運用中のシステムにテスト機能を組み込み、システムを利用している大勢のユーザーの行動を観察し統計的に行動傾向を把握することでユーザーテストとすることが多い

note:A/Bテストは広義の[観察]調査であり、「なぜそのような行動を取るのか？」まではわからない。行動の理由を深く知るためには[インタビュー]調査を併用する必要がある
