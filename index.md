...
note:製品サービスを用いない作業はユーザータスクではない

note:ただし、ユーザータスクの準備作業として製品サービスを用いない作業が発生することはあり得る。そのような作業はユーザータスクの具体的手順の一部と解釈する(例：電車に乗るために駅まで歩く)(例：包丁でりんごの皮を剥く前に手を洗う)
...
note:ひとつの[ユーザータスク]からはひとつ以上のユーザー要求が発生する

note:ユーザー要求は｢ユーザーはこの[製品サービス]を用いて、[利用文脈]〇〇の元で、[ユーザータスク]〇〇を達成できること｣の形で表現する

note:あるユーザータスクにおいて、ユーザーにとって重要な[利用文脈]がなく、｢あらゆる状況で…｣としか言えない場合は、｢ユーザーはこの[製品サービス]を用いて、[ユーザータスク]〇〇を達成できること｣だけがそのユーザータスクに対するユーザー要求になる
