...
UXデザインにおける諸々の専門用語を集めた用語集です（全259語）。代表的なUXデザイン手法のみをカバーしています。行動設計に関する用語については別ページにまとめました（[→UX行動設計用語集](../ux-behavioral-design-glossary/)）
...
2つの試作物の優劣比較を目的とした[ユーザーテスト]

[A/Bテスト]:#A/Bテスト "2つの試作物の優劣比較を目的としたユーザーテスト"

[ABテスト]:#A/Bテスト "（→A/Bテスト）2つの試作物の優劣比較を目的としたユーザーテストト"

note:A/Bテストの具体的な用途としては、複数のUI案の使いやすさの優劣比較、複数のウェブ広告案の広告効果の優劣比較、などが挙げられる

note:A/Bテストは、本番運用中のシステムにテスト機能を組み込み、システムを利用している大勢のユーザーの行動を観察し統計的に行動傾向を把握することでユーザーテストとすることが多い

note:A/Bテストで行動傾向を把握することはできてもその行動の背景にある[利用文脈]まではわからない。利用文脈を知るためには[インタビュー]調査を併用する必要がある
...
マーケティングの用語で、ユーザーがユーザーゴール達成に至るまでの過程

[カスタマージャーニー]:#カスタマージャーニー "マーケティングの用語で、ユーザーが製品サービスを利用して[ユーザーゴール]に至る過程で得るさまざまな体験を「旅」に例えたもの"

note:[UX]（ユーザー体験）は断片的な短時間の体験だけに注目することがあるが、カスタマージャーニーは必ずユーザーゴールに至る過程の全体を対象とする。
...
「きっかけ→行動→[報酬]」を繰り返すことで行動を習慣化できる、とする理論（例：＜起床する→歯を磨く→爽快感を得る＞の繰り返しで歯磨きが習慣化する）

[習慣ループ理論]:#習慣ループ理論 "｢きっかけ→行動→報酬｣を繰り返すことで行動を習慣化できる、とする理論（例：＜起床する→歯を磨く→爽快感を得る＞の繰り返しで歯磨きが習慣化する）"

note:金銭に換算できないものも報酬になり得る。たとえば｢褒められる｣という体験は報酬になり得る
...
[ナッジ]を悪用し、ユーザーにとって不利な行動をユーザーに取らせるよう誘導する行為

[スラッジ]:#スラッジ "ナッジを悪用し、ユーザーにとって不利な行動をユーザーに取らせるよう誘導する行為"

note:ウェブデザイン業界では「[ダークパターン]」と呼ばれる
...
人間の定期的な往来により芝生や未舗装地に自然発生した道。転じて、設計意図に反して自然発生したユーザー行動の[動線]

[デザイア・パス]:#デザイア・パス "人間の定期的な往来により芝生や未舗装地に自然発生した道。転じて、設計意図に反して自然発生したユーザー行動の動線"

[けもの道]:#デザイア・パス "（→デザイア・パス）人間の定期的な往来により芝生や未舗装地に自然発生した道。転じて、設計意図に反して自然発生したユーザー行動の動線"

[欲望の道]:#デザイア・パス "（→デザイア・パス）人間の定期的な往来により芝生や未舗装地に自然発生した道。転じて、設計意図に反して自然発生したユーザー行動の動線"

note:人間は無意識に近道を好む心理的傾向があるため、歩道があるのに敢えて芝生を横切って目的地に向かうことがある。大勢の人間がくりかえし芝生を横切って二地点間を往来することで、芝生が踏み荒らされ、道が自然発生する

note:同様に、人間は無意識に近道を好む心理的傾向があるため、製品仕様としての正解操作手順をユーザーが守るとは限らない。また、製品を想定外の用途に用いるユーザーが想定外の用途のための正解操作手順を独自に確立することがある
...
ゲームアプリやWebサイトにおいて、ユーザー行動の軌跡を示す仮想の線。ページ遷移やページスクロール、ユーザーの視線移動など

[動線]:#動線 "ゲームアプリやWebサイトにおいて、ユーザー行動の軌跡を示す仮想の線。ページ遷移やページスクロール、ユーザーの視線移動など"

note:元々は建築業界の用語で、建物内や敷地内で人や物が移動する経路を示す仮想の線のこと
...
見た目の魅力的な[製品サービス]は[使いやすさ]にも優れているだろうと思い込むユーザーの心理的傾向

[美的ユーザビリティ効果]:#美的ユーザビリティ効果 "見た目の魅力的な製品サービスは使いやすさにも優れているだろうと思い込むユーザーの心理的傾向"

note:[品質評価]においては、美的ユーザビリティ効果は[利用時品質]の測定における「ノイズ」であり、慎重に排除する必要がある

note:美的ユーザビリティ効果といえども、タスク達成不可能レベルの重大な使いづらさは隠蔽不可能である。見た目の魅力的な製品サービスであっても使いづらさは改善されるべきである
...
マーケティングの用語で、ユーザーが[ユーザーゴール]達成のために行う一連の行動を、[利用文脈]の大きく変わる節目で切り分けたもの

[フェーズ]:#フェーズ "マーケティングの用語で、ユーザーが[ユーザーゴール]達成のために行う一連の行動を、[利用文脈]の大きく変わる節目で切り分けたもの"

note:(関連語)[利用文脈]、[カスタマージャーニーマップ]
...
[フォグの消費行動モデル]:#フォグの消費行動モデル "人が行動を起こすに至る条件を「Motivation(動機)」「Ability(実行能力)」「Trigger(きっかけ)」の3要素で模式化したもの"
...
note:ひとつの[ユーザータスク]からはひとつ以上のユーザー要求が発生する。まず基本的要求として｢ユーザーはこの製品サービスを用いて、**ユーザータスク〇〇を達成できること**｣が発生し、さらに追加要求として｢ユーザーはこの製品サービスを用いて、**追加条件〇〇の元で**、ユーザータスク〇〇を達成できること｣が発生する
...
基本的要求「メッセージを送信できること」
　↓
追加要求「電車移動中に、メッセージを送信できること」
追加要求「片手操作で、メッセージを送信できること」
追加要求「音を出さず周囲の乗客に迷惑をかけずに、メッセージを送信できること」
...
note:品質特性としてのユーザビリティは、ISO/IEC 25010では｢[利用時品質]｣としてさらに詳細に定義されている
...
[利用時品質]:#利用時品質 "特定の利用者が特定の利用状況において、有効性・効率性・リスク回避性及び満足性に関して特定の目標を達成するためのニーズを満たすために、製品又はシステムを利用できる度合い(ISO/IEC 25010の定義)"
