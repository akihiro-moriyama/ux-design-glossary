...
note:｢ユーザータスク｣は、システム工学における｢[ユースケース]｣とほぼ同じ意味である。ユーザータスク一覧表はUMLユースケース図書式で作図できる

note:｢インタラクション記述｣は、UXデザインにおけるシステム工学における｢[ユースケース記述] (のうちの特にアクションシーケンス)｣とほぼ同じ意味である。インタラクション記述はUMLシーケンス図書式で作図できる
...
note:[要求]も[要件]も英語ではrequirementであり同一である。日本語では｢要求＝ユーザーの要求したもの｣｢要件＝設計者が技術的合理性を勘案して要求を再定義したもの｣の意味合いで使い分ける

note:UXデザインでは、[ユーザー要求]は[理想シナリオ]の記述から考察して得る。ユーザー調査結果から直接得るものではない。

note:(関連語)[非機能要求]、(関連語)[ユーザー要求]
...
[顧客価値]:#顧客価 "製品サービスの使用に関する体験を通じて顧客の得る価値"
...
note:デザインは芸術ではない。デザインは必ずなんらかの問題解決を目的として行われる行為である

note:デザインにおいては、見た目の美しさや格好良さは、問題解決の手段のひとつに過ぎない
...
note:｢ユーザータスク｣は、システム工学における｢[ユースケース]｣とほぼ同じ意味である。ユーザータスク一覧表はUML[ユースケース図]書式で作図できる
...
note:まれに、[ユーザータスク]の上位にある[ユーザーゴール]からユーザー要求が発生する場合がある。[ユーザータスク]と[ユーザーゴール]とは小目標と大目標の関係にある
...
システム工学の用語で、ユーザーの目的達成のためにユーザーとシステムとの間で交わされるやりとりを表現したもの
...
note:ユースケースは、ユースケース一覧表とユースケース記述で文書化されるが、これらはUMLユースケース図とUMLシーケンス図で代用されることがある

note:ひとくちにユーザーといっても実際にはシステムとの関わり方の違いによって多様に分類され、さらには人間でない別システムがシステムを利用することもある。ユーザーが多様であることを強調するため、システム工学ではユーザーの代用語として｢アクター｣という用語を用いる

note:｢ユースケース｣は、UXデザインにおける｢[ユーザータスク]｣とほぼ同じ意味である
...
note:[ユースケース]は、ユースケース一覧表とユースケース記述で文書化されるが、これらはUMLユースケース図とUMLシーケンス図で代用されることがある
...
note:一般的には、UML(統一モデリング言語)で定義されたユースケース図記法に従って作図する

### 要求

→[ユーザー要求]

note:要求も[要件]も英語ではrequirementであり同一である。日本語では｢要求＝ユーザーの要求したもの｣｢要件＝設計者が技術的合理性を勘案して要求を再定義したもの｣の意味合いで使い分ける

### 要件

→[設計要件]

note:[要求]も要件も英語ではrequirementであり同一である。日本語では｢要求＝ユーザーの要求したもの｣｢要件＝設計者が技術的合理性を勘案して要求を再定義したもの｣の意味合いで使い分ける
